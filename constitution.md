# [hwcloud-controlpanel] 项目开发宪法
# Version: 1.0, Ratified: 2026-02-26
...
- **1.2 (标准库优先):** 除非有极其充分的理由，否则必须优先使用Go标准库。例如，Web服务使用`net/http`，而不是Gin。
...
- **2.3 (拒绝Mocks):** 优先编写集成测试，使用真实的依赖或fake object，而不是过度依赖Mock。
...
- **4.1 (包的内聚):** `internal`目录下的各个包应保持高度内聚和低耦合。例如，`huawei`包只负责使用华为云SDK（huaweicloud-sdk-go-v3）与华为云API交互，绝不能包含其他业务逻辑。
